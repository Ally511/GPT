...
- backoff_prob: calculates backoff probabilites, called in perplexity and generate
- perplexity: calculates perplexity of a given text based on n-gram probabilities
- generate: generates a sequence of tokens
...
class N_gram:
  ...
  def get_unigram_probs(self, corpus):
    """
    Pass corpus (already to byte-pair) and get unigram probabilities.
    """
    c = Counter(corpus)
    total = sum(c.values())
    # add count of 1 for Laplace Smoothing
    unigram_probs = {token: (count+1) / (total+self.vocab_size) for token, count in c.items()} 
  ...
  def calculate_n_gram_probs(self, split_text):
    """
    Builds conditional probabilities P(w_n | w_1, ..., w_{n-1}) using nested dictionaries.
    """
    n = self.ndim
    n_gram_probs = []
    for i in range(n):
  ...
  def backoff_prob(self, context, token, n, generate = False):
    """
    Recursively back off from the full (n-1)-gram context down to unigram.
    """
    # if at the top order (len(context)==n-1), try to use that
    if n > 1:
      ...
      # if back off possible, drop the first item in context
      if len(context) > 0:
    ...
    # if still unseen, return prob=floor
    return self.unigram_probs.get(token, self.floor)


  def perplexity(self, text, n):
    """
    Compute PP = exp{-1/M ∑_i log P(w_i | context_i)}
    """
    log_prob = 0.0
    M = 0
    floor = self.floor
  ...
  def generate(self, n, max_length=100, seed=None, k = 5):
    ...
    Args:
      n (list of str): Tokenised prompt
      max_length (int): Maximum amount of tokens to generate
      seed (int): For reproducability
      k (int): k to consider for top_k sampling
    """
